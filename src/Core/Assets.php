...
final class Assets {
	 ...
	 */
	public const ADMIN_RUNTIME_HANDLE = 'mpfbs-admin-runtime';

	/**
	 * Handle prefix used for the exported Next.js chunks.
	 */
	private const ADMIN_CHUNK_HANDLE = 'mpfbs-admin-chunk-';
	 ...
	 * them by MPFBS_VERSION means an edit that does not also bump the plugin
	...
	private static function asset_version( string $relative ): string {
		$path     = MPFBS_PATH . ltrim( $relative, '/' );
		$modified = is_readable( $path ) ? filemtime( $path ) : false;

		return false === $modified ? MPFBS_VERSION : MPFBS_VERSION . '.' . (string) $modified;
	}
	...
	public function enqueue_admin_app(): void {
		wp_enqueue_style(
			'mpfbs-admin',
			MPFBS_URL . 'assets/admin/css/mpfbs-admin.css',
			array(),
			self::asset_version( 'assets/admin/css/mpfbs-admin.css' )
		);
		...
		foreach ( $this->renderer->styles() as $index => $url ) {
			...
			wp_enqueue_style( 'mpfbs-admin-app-' . $index, $url, array( 'mpfbs-admin' ), null );
		}

		wp_enqueue_script(
			self::ADMIN_RUNTIME_HANDLE,
			MPFBS_URL . 'assets/admin/js/mpfbs-admin-runtime.js',
			array(),
			self::asset_version( 'assets/admin/js/mpfbs-admin-runtime.js' ),
			true
		);

		wp_add_inline_script(
			self::ADMIN_RUNTIME_HANDLE,
			'window.mpfbsAdmin = ' . wp_json_encode( $this->admin_config() ) . ';',
			'before'
		);
	...
	public function admin_config(): array {
		$config = array(
			...
			'restUrl'      => esc_url_raw( trailingslashit( rest_url( 'mpfbs/v1' ) ) ),
			'restNonce'    => wp_create_nonce( 'wp_rest' ),
			'adminUrl'     => esc_url_raw( admin_url() ),
			'pageUrl'      => esc_url_raw( admin_url( 'admin.php?page=mpfbs-dashboard' ) ),
			'assetUrl'     => esc_url_raw( trailingslashit( MPFBS_URL . 'assets/' ) ),
		 ...
		 */
		return (array) apply_filters( 'mpfbs_admin_config', $config );
	}
	...
	private function translations(): array {
		$strings = array(
			...
			'How many of each, then a name for every one of them.' => __( 'How many of each, then a name for every one of them.', 'magepeople-ferry-booking-system' ),
			...
			'Shown on booking confirmations.'              => __( 'Shown on booking confirmations.', 'magepeople-ferry-booking-system' ),
			...
			'Every traveller needs a name.' => __( 'Every traveller needs a name.', 'magepeople-ferry-booking-system' ),
			...
			'Short identifier shown on booking confirmations, for example NAP.' => __( 'Short identifier shown on booking confirmations, for example NAP.', 'magepeople-ferry-booking-system' ),
			...
			'Shown to passengers on their booking confirmation.'         => __( 'Shown to passengers on their booking confirmation.', 'magepeople-ferry-booking-system' ),
			...
			'Appears on booking confirmations, for example CAR.' => __( 'Appears on booking confirmations, for example CAR.', 'magepeople-ferry-booking-system' ),
			'Appears on booking confirmations, for example ADULT.' => __( 'Appears on booking confirmations, for example ADULT.', 'magepeople-ferry-booking-system' ),
			...
			'Shown on the price breakdown, for example VAT or GST.' => __( 'Shown on the price breakdown, for example VAT or GST.', 'magepeople-ferry-booking-system' ),
			...
			'Bookings made on the website or by staff appear here.' => __( 'Bookings made on the website or by staff appear here.', 'magepeople-ferry-booking-system' ),
			...
			'An administrator is still setting up the ferry operation.' => __( 'An administrator is still setting up the ferry operation.', 'magepeople-ferry-booking-system' ),
			...
			'How your business appears on confirmations and emails. Check what is filled in and correct anything that is wrong.' => __( 'How your business appears on confirmations and emails. Check what is filled in and correct anything that is wrong.', 'magepeople-ferry-booking-system' ),
		 ...
		 */
		return (array) apply_filters( 'mpfbs_js_translations', $strings );
	}
}
