...
export function Steps( { steps, current }: { steps: Step[]; current: string } ): JSX.Element {
	...
	return (
		<ol className="mpfbsb-steps" aria-label="progress">
			{ steps.map( ( step, position ) => {
				const state = position < index ? 'done' : position === index ? 'current' : 'todo';

				return (
					<li key={ step.id } className={ `mpfbsb-steps__item is-${ state }` } aria-current={ state === 'current' ? 'step' : undefined }>
						<span className="mpfbsb-steps__marker" aria-hidden="true">
							{ state === 'done' ? (
						...
						<span className="mpfbsb-steps__label">{ step.label }</span>
					</li>
				);
			} ) }
		</ol>
